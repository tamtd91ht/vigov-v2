...
// erases exactly that. Running docstore.LoaiVanBanStore unchanged over this driver keeps the
...
func dungUseCase(t *testing.T, k *khoGia) (*DanhMucLoaiVanBan, context.Context) {
	t.Helper()
	db := sql.OpenDB(k)
	...
	kho := store.New(db)
	uc := NewDanhMucLoaiVanBan(kho, docstore.NewLoaiVanBanStore(kho))
