...
func nguoiThuLoaiNhiemVu() audit.Actor {
	return audit.Actor{ID: "nd-01JINTERNALIDCUACANBO", Kind: "staff", IP: "10.0.0.7"}
}

func themMauLoaiNhiemVu() YeuCauThemLoaiNhiemVu {
	return YeuCauThemLoaiNhiemVu{Ma: "bao-cao", Nhan: "Báo cáo", ThuTu: 5}
}

// dongTang1 is a row the commune added itself — tier 1, the only tier that may be deleted.
func dongTang1LoaiNhiemVu() *hangLoaiNhiemVu {
	return &hangLoaiNhiemVu{id: "lnv-001", ma: "bao-cao", nhan: "Báo cáo", dangDung: true,
		thuTu: 5, nguon: domain.NguonDonVi}
}

// dongTang2 ships with the software: it may be disabled and relabelled, never deleted.
func dongTang2LoaiNhiemVu() *hangLoaiNhiemVu {
	return &hangLoaiNhiemVu{id: "lnv-002", ma: "cong-van", nhan: "Công văn", dangDung: true,
		thuTu: 1, nguon: domain.NguonHeThong}
}

// dongTang3 is a system row the SOURCE CODE BRANCHES ON. Relabelling is all that is left.
func dongTang3LoaiNhiemVu() *hangLoaiNhiemVu {
	return &hangLoaiNhiemVu{id: "lnv-003", ma: "quyet-dinh", nhan: "Quyết định", dangDung: true,
...
func TestThemGhiDongVaVetTrongCUNGMotGiaoDich(t *testing.T) {
	...
	if !k.coCau("INSERT INTO loai_nhiem_vu") {
...
func TestThemVetMangDuNguoiViecVaMa(t *testing.T) {
	...
	doi := chuoiTrongDoiLoaiNhiemVu(vet[0].args)
	for _, muon := range []string{string(xaLoaiNhiemVuA), "nd-01JINTERNALIDCUACANBO", "staff", "10.0.0.7",
		HanhViThemLoaiNhiemVu, "bao-cao"} {
		if !doi[muon] {
...
func TestThemKhongGhiGiKhiVetHong(t *testing.T) {
	...
	k := &khoLoaiNhiemVuGia{loiSau: "INSERT INTO audit_log"}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)
...
func TestXoaChiChoTang1(t *testing.T) {
	for ten, tc := range map[string]struct {
		hang *hangLoaiNhiemVu
		loi  error
	}{
		"tầng 1 xoá được":       {dongTang1LoaiNhiemVu(), nil},
		"tầng 2 không xoá được": {dongTang2LoaiNhiemVu(), domain.ErrKhongXoaDuocMucHeThong},
		"tầng 3 không xoá được": {dongTang3LoaiNhiemVu(), domain.ErrKhongXoaDuocMucHeThong},
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoLoaiNhiemVuGia{hang: tc.hang}
			uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

			err := uc.Xoa(ctx, tc.hang.id, "gộp vào loại khác", nguoiThuLoaiNhiemVu())
			if tc.loi == nil {
				if err != nil {
			...
			if k.coCau("UPDATE loai_nhiem_vu") {
...
func TestTatChiBiTuChoiOTang3(t *testing.T) {
	...
	for ten, tc := range map[string]struct {
		...
		"tầng 1 tắt được":       {dongTang1LoaiNhiemVu(), true},
		"tầng 2 tắt được":       {dongTang2LoaiNhiemVu(), true},
		"tầng 3 không tắt được": {dongTang3LoaiNhiemVu(), false},
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoLoaiNhiemVuGia{hang: tc.hang}
			uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

			_, err := uc.Sua(ctx, tc.hang.id, YeuCauSuaLoaiNhiemVu{DangDung: &sai}, nguoiThuLoaiNhiemVu())
			if tc.duoc {
				if err != nil {
					t.Fatalf("tắt lỗi: %v", err)
				}
				if !k.coCau("UPDATE loai_nhiem_vu") || !k.coCau("INSERT INTO audit_log") {
...
func TestSuaNhanDuocOMoiTang(t *testing.T) {
	...
	nhan := "Quyết định (sửa)"
	for ten, hang := range map[string]*hangLoaiNhiemVu{
		"tầng 1": dongTang1LoaiNhiemVu(), "tầng 2": dongTang2LoaiNhiemVu(), "tầng 3": dongTang3LoaiNhiemVu(),
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoLoaiNhiemVuGia{hang: hang}
			uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

			sau, err := uc.Sua(ctx, hang.id, YeuCauSuaLoaiNhiemVu{Nhan: &nhan}, nguoiThuLoaiNhiemVu())
			if err != nil {
...
func TestCapNhatKhongDungToiMaVaNguon(t *testing.T) {
	...
	nhan := "Công văn mới"
	k := &khoLoaiNhiemVuGia{hang: dongTang2LoaiNhiemVu()}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

	if _, err := uc.Sua(ctx, "lnv-002", YeuCauSuaLoaiNhiemVu{Nhan: &nhan}, nguoiThuLoaiNhiemVu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	capNhat := k.cau("UPDATE loai_nhiem_vu")
	if len(capNhat) != 1 {
...
func TestXoaLaXoaMemVaGhiDuBaCot(t *testing.T) {
	...
	if err := uc.Xoa(ctx, "lnv-001", "gộp vào loại khác", nguoiThuLoaiNhiemVu()); err != nil {
		t.Fatalf("Xoá lỗi: %v", err)
	}
	...
	doi := chuoiTrongDoiLoaiNhiemVu(xoa[0].args)
...
func TestMoiDuongDocGhiDeuLoaiDongDaXoaMem(t *testing.T) {
	...
	nhan := "Báo cáo mới"
	k2 := &khoLoaiNhiemVuGia{hang: dongTang1LoaiNhiemVu()}
	uc2, ctx2 := dungUseCaseLoaiNhiemVu(t, k2)
	if _, err := uc2.Sua(ctx2, "lnv-001", YeuCauSuaLoaiNhiemVu{Nhan: &nhan}, nguoiThuLoaiNhiemVu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	for _, l := range k2.cau("UPDATE loai_nhiem_vu") {
...
func TestSuaDongDaXoaLa404(t *testing.T) {
	...
	nhan := "Báo cáo mới"
	k := &khoLoaiNhiemVuGia{hang: nil}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

	if _, err := uc.Sua(ctx, "lnv-001", YeuCauSuaLoaiNhiemVu{Nhan: &nhan}, nguoiThuLoaiNhiemVu()); !errors.Is(err, docstore.ErrDanhMucKhongTonTai) {
...
func TestXoaDoiLyDo(t *testing.T) {
	...
	if err := uc.Xoa(ctx, "lnv-001", "   ", nguoiThuLoaiNhiemVu()); !errors.Is(err, domain.ErrThieuLyDoXoa) {
...
func TestThemTuChoiMaDaCoKeCaDongDaXoa(t *testing.T) {
	...
	k := &khoLoaiNhiemVuGia{maTrung: 1}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

	_, err := uc.Them(ctx, themMauLoaiNhiemVu(), nguoiThuLoaiNhiemVu())
...
func TestThemTuChoiKhiDayTran(t *testing.T) {
	...
	// past TranDanhMucLoaiNhiemVu, so the row that crosses the line does not merely add itself — it
	// turns the whole catalogue into a 500 for every screen in the commune.
	k := &khoLoaiNhiemVuGia{dem: docstore.TranDanhMucLoaiNhiemVu}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)
	...
	// perfectly valid.
	k2 := &khoLoaiNhiemVuGia{dem: docstore.TranDanhMucLoaiNhiemVu - 1}
	uc2, ctx2 := dungUseCaseLoaiNhiemVu(t, k2)
	if _, err := uc2.Them(ctx2, themMauLoaiNhiemVu(), nguoiThuLoaiNhiemVu()); err != nil {
...
func TestSuaKhongDoiGiThiKhongGhiVaKhongVet(t *testing.T) {
	...
	nhan := "Báo cáo" // exactly what dongTang1LoaiNhiemVu() already holds
	k := &khoLoaiNhiemVuGia{hang: dongTang1LoaiNhiemVu()}
	uc, ctx := dungUseCaseLoaiNhiemVu(t, k)

	sau, err := uc.Sua(ctx, "lnv-001", YeuCauSuaLoaiNhiemVu{Nhan: &nhan}, nguoiThuLoaiNhiemVu())
	if err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	if sau.Nhan != "Báo cáo" {
...
func TestSuaDatMacDinhBoMacDinhCuTruocTrongCungGiaoDich(t *testing.T) {
	...
	if _, err := uc.Sua(ctx, "lnv-001", YeuCauSuaLoaiNhiemVu{LaMacDinh: &dung}, nguoiThuLoaiNhiemVu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
...
func TestMoiCauLenhMangXaTuContext(t *testing.T) {
	...
	// statement forgot.
	for _, xa := range []string{string(xaLoaiNhiemVuA), string(xaLoaiNhiemVuB)} {
		k := &khoLoaiNhiemVuGia{}
		uc, _ := dungUseCaseLoaiNhiemVu(t, k)
...
func TestKhongCoXaTrongContextThiPanic(t *testing.T) {
	...
	k := &khoLoaiNhiemVuGia{}
	uc, _ := dungUseCaseLoaiNhiemVu(t, k)
	_, _ = uc.Them(context.Background(), themMauLoaiNhiemVu(), nguoiThuLoaiNhiemVu())
}
...
func chuoiTrongDoiLoaiNhiemVu(args []driver.Value) map[string]bool {
	ra := map[string]bool{}
	for _, a := range args {
		switch v := a.(type) {
		case string:
