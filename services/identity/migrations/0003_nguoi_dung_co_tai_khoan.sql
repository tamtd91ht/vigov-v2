...
-- pkg/migrate compares the checksum of every applied file at startup. Editing it would stop
...
--   2. IF IT STOPS HALF-WAY: it cannot land half-applied. pkg/migrate runs each file in ONE
...
-- pkg/migrate package comment; the threshold for needing it is a statement whose lock time is
...
-- is atomic here: pkg/migrate wraps each file in a single transaction, so there is no window in
...
-- knowing that an empty hash cannot sign in today by accident: pkg/password.KiemTra cannot parse
...
--
-- pkg/migrate has no rollback on purpose: undoing DDL on archival records is an administrative
