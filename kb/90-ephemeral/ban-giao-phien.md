...
| Biên bản họp: **dự thảo → đã ký** (ký bằng `task.approve`, khoá sau ký, sai thì **biên bản bổ sung**); thư ký + số/ngày Thông báo kết luận chép tay; trạng thái kết luận **suy ra** + dấu "không phát sinh"; sắp theo ngày họp; URL `…/signature`, `…/no-task-marker`; `POST /tasks` không nhận nguồn `ket-luan-hop` | 25/09 | người dùng | `service-petitions/bien-ban-hop-tang-du-lieu` |
...
| `make kb | grep …` "xanh" mà hợp đồng KHÔNG đổi | Ống dẫn che mã thoát: `tools/apidoc` từng từ chối sinh vì tên trường `secretary` dính bộ dò bí mật, hai commit trôi qua với openapi.json cũ. Luôn chạy `make kb > log; echo $?`. Chạy ĐỦ `make check` (gồm `lint`) — lượt Thu chi sót `lint` để lọt một tệp chưa gofmt |
