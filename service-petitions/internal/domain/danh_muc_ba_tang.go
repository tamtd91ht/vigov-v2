...
// migrations/0003_danh_muc_loai_nhiem_vu.sql, function `danh_muc_ba_tang`, a BEFORE UPDATE OR
...
// `loai_nhiem_vu_re_nhanh_thi_he_thong` refuses ma_nguon_re_nhanh on a `don-vi` row, because the
...
// Tang of one catalogue row. ONE METHOD PER CATALOGUE THIS SERVICE OWNS — petitions owns two, and
// one shared helper taking two loose columns would be a helper every call site could hand the wrong
// pair to.
func (l LoaiNhiemVu) Tang() Tang { return TangCua(l.Nguon, l.MaNguonReNhanh) }

func (m MucUuTienNhiemVu) Tang() Tang { return TangCua(m.Nguon, m.MaNguonReNhanh) }
