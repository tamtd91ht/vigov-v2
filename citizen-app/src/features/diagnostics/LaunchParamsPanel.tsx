 ...
 *
 * KHÔNG PHẢI MỘT TÍNH NĂNG, và người dân không bao giờ thấy nó: chỉ hiện khi app được mở kèm
 * `debug`. Nó tồn tại để trả lời nốt một câu còn treo của ADR 0018 — tham số deep link có giữ
 * khi app chạy nền rồi quay lại không — bằng cách đo trên máy thật thay vì suy đoán tiếp.
 *
 * MỘT CỘT, KHÔNG PHẢI HAI — và cột bị bỏ là cột đã trả lời xong câu hỏi của nó:
 *
 *   Bản trước in cạnh nhau "Theo SDK (`getRouteParams`)" và "Theo URL (`location.search`)",
 *   để so hai nguồn. Ngày 18/09/2026 phép so ấy cho kết quả: **hai cột giống hệt nhau**. Thế là
 *   `zmp-sdk` bỏ được, và nó đã được bỏ — bundle từ 513 kB xuống 246 kB. Giữ lại cột SDK bây giờ
 *   nghĩa là nhập lại đúng thư viện vừa gỡ, chỉ để in ra một cột mà ta đã biết đáp án.
 *
 * CÁCH ĐO PHẦN CÒN LẠI:
 *
 *   1. Mở link `https://zalo.me/s/<APP_ID>/?env=TESTING&version=<n>&debug=1&t=<ULID>&src=qr`
 *      → xem bảng này liệt kê đủ các tham số không.
 ...
 *      Lúc ấy app phải rơi vào màn chọn xã chứ không được hỏng — đó là đường ADR 0005 bắt buộc.
 *
...
export function LaunchParamsPanel({ thamSo }: Props) {
  ...
  return (
    <section className="chan-doan" aria-label="Thông tin chẩn đoán">
      ...
      {/* MỘT NGUỒN DUY NHẤT, và đó là kết quả của phép đo chứ không phải sự cắt bớt: cột này
          từng được đặt cạnh cột SDK để so, và hai cột giống hệt nhau. Xem lib/launch-params.ts. */}
