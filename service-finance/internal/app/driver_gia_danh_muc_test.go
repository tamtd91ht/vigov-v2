...
// erases exactly that. Running docstore.HangMucKeHoachVonStore unchanged over this driver keeps the
...
func dungUseCase(t *testing.T, k *khoGia) (*DanhMucHangMuc, context.Context) {
	t.Helper()
	db := sql.OpenDB(k)
	...
	kho := store.New(db)
	uc := NewDanhMucHangMuc(kho, docstore.NewHangMucKeHoachVonStore(kho))
