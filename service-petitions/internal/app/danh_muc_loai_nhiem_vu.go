...
// the customer's call, not a use case's — see migrations/0003_danh_muc_loai_nhiem_vu.sql:47-52. An
...
// KhoLoaiNhiemVu is the store, declared at the point of use.
//
...
type KhoLoaiNhiemVu interface {
	TheoIDDeSua(ctx context.Context, tx *store.ScopedTx, id string) (domain.LoaiNhiemVu, error)
	...
	Chen(ctx context.Context, tx *store.ScopedTx, lnv domain.LoaiNhiemVu) error
	BoMacDinhKhac(ctx context.Context, tx *store.ScopedTx, trongID string) error
	CapNhat(ctx context.Context, tx *store.ScopedTx, lnv domain.LoaiNhiemVu) error
	XoaMem(ctx context.Context, tx *store.ScopedTx, id, boi, lyDo string) error
}

// DanhMucLoaiNhiemVu owns adding, editing and retiring one commune's document types.
type DanhMucLoaiNhiemVu struct {
	db  *store.DB
	kho KhoLoaiNhiemVu
...
func NewDanhMucLoaiNhiemVu(db *store.DB, kho KhoLoaiNhiemVu) *DanhMucLoaiNhiemVu {
	return &DanhMucLoaiNhiemVu{db: db, kho: kho, sinhID: ulid.Moi}
}
...
// been edited again.
const (
	HanhViThemLoaiNhiemVu = "them_loai_nhiem_vu"
	HanhViSuaLoaiNhiemVu  = "sua_loai_nhiem_vu"
	HanhViXoaLoaiNhiemVu  = "xoa_loai_nhiem_vu"
)

// YeuCauThemLoaiNhiemVu is one new row, as it arrives from the handler.
//
...
type YeuCauThemLoaiNhiemVu struct {
	Ma        string
	Nhan      string
	ThuTu     int
	LaMacDinh bool
...
type YeuCauSuaLoaiNhiemVu struct {
	Nhan      *string
	ThuTu     *int
	DangDung  *bool
	LaMacDinh *bool
...
func (uc *DanhMucLoaiNhiemVu) Them(ctx context.Context, yc YeuCauThemLoaiNhiemVu,
	...
	id, err := uc.sinhID()
	if err != nil {
		return domain.LoaiNhiemVu{}, fmt.Errorf("danh_muc_loai_nhiem_vu: sinh mã: %w", err)
	}
	...
	err = uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if n >= docstore.TranDanhMucLoaiNhiemVu {
		...
		delta, err := json.Marshal(map[string]any{"sau": tomTatLoaiNhiemVu(moi)})
		if err != nil {
			return fmt.Errorf("danh_muc_loai_nhiem_vu: mã hoá delta: %w", err)
		}
	...
	if err != nil {
		...
		return domain.LoaiNhiemVu{}, boc(ctx, "thêm", err)
	}
	return moi, nil
}
...
func (uc *DanhMucLoaiNhiemVu) Sua(ctx context.Context, id string, yc YeuCauSuaLoaiNhiemVu,
	nguoi audit.Actor) (domain.LoaiNhiemVu, error) {

	if id == "" {
		return domain.LoaiNhiemVu{}, docstore.ErrDanhMucKhongTonTai
	}
	...
	err := uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if khongDoiLoaiNhiemVu(truoc, sau) {
			return nil
		}
		...
		delta, err := json.Marshal(map[string]any{
			"truoc": tomTatDoiLoaiNhiemVu(truoc, sau, true),
			"sau":   tomTatDoiLoaiNhiemVu(truoc, sau, false),
		})
		if err != nil {
			return fmt.Errorf("danh_muc_loai_nhiem_vu: mã hoá delta: %w", err)
		}
		return audit.Write(ctx, tx, audit.Entry{
			Actor:   nguoi,
			Action:  HanhViSuaLoaiNhiemVu,
			Subject: sau.Ma,
			Delta:   delta,
		})
	})
	if err != nil {
		return domain.LoaiNhiemVu{}, boc(ctx, "sửa", err)
	}
	return sau, nil
}
...
func (uc *DanhMucLoaiNhiemVu) Xoa(ctx context.Context, id, lyDoTho string, nguoi audit.Actor) error {
	if id == "" {
	...
	if nguoi.ID == "" {
		...
		return fmt.Errorf("danh_muc_loai_nhiem_vu: thiếu người xoá")
	}
...
func tomTatLoaiNhiemVu(l domain.LoaiNhiemVu) map[string]any {
...
func tomTatDoiLoaiNhiemVu(truoc, sau domain.LoaiNhiemVu, ben bool) map[string]any {
	ra := map[string]any{}
