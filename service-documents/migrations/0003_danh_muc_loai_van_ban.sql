-- documents — the reference catalogue documents owns: document type (ADR 0024).
--
...
--      and then the first document record that stores one of these codes as a value.
--   5. RETENTION: nothing is removed. A document type is part of how a document was registered
--      and numbered; the incoming/outgoing books outlive any tidying up of the catalogue, so
--      rows are soft deleted and hard DELETE is refused by a trigger rather than by convention.
--
-- ---------------------------------------------------------------------------
...
--     document already registered under the old value then silently reads as the new one.
--     Nothing errors; the type shown on old records is simply wrong from then on, and document
--     numbering follows the type (ADR 0024).
--
...
--   `ma`              — an issued code is never renumbered (rule 7, invariant 3). Document
...
BEGIN
    ...
    IF NEW.ma IS DISTINCT FROM OLD.ma THEN
        RAISE EXCEPTION 'catalogue %: `ma` is immutable', TG_TABLE_NAME
            USING HINT = 'An issued code is never renumbered (rule 7, invariant 3). Document '
                         'records hold this code AS A VALUE and nothing rewrites them. '
...
-- @scope:  tenant
--
-- loai_van_ban — `cong-van`, `quyet-dinh`, `thong-bao`, and the rest of the commune's list.
--
-- Owned by documents because the incoming/outgoing books live here and numbering follows the
-- type (ADR 0024).
--
-- TIER 2 FOR THE SHIPPED CODES, on the evidence available: the specification uses the type to
-- classify and to drive numbering, and numbering is configuration attached to the type rather
-- than a branch keyed on one particular code. Nothing written down today says "if the type is
-- `quyet-dinh` then ...". If such a branch appears, that one code moves to tier 3 in a
-- migration — the guard allows that direction and refuses the other.
--
-- WHAT WOULD CHANGE THE ANSWER, so it is checked rather than assumed: a numbering series whose
-- reset rule, prefix or register differs BY CODE rather than by a row of configuration. That is
-- a branch on the code, it touches issued document numbers, and issued numbers are the one
-- thing rule 7 never lets us renumber.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS loai_van_ban (
    tenant_id         TEXT        NOT NULL,
    id                TEXT        NOT NULL,
    ma                TEXT        NOT NULL,          -- "cong-van"
    nhan              TEXT        NOT NULL,          -- "Công văn"
...
DO $$ BEGIN
    FOR i IN 0..31 LOOP
        EXECUTE format(
            'CREATE TABLE IF NOT EXISTS loai_van_ban_p%s PARTITION OF loai_van_ban '
...
-- (rule 7, invariant 2).
CREATE INDEX IF NOT EXISTS loai_van_ban_danh_sach
    ON loai_van_ban (tenant_id, thu_tu) WHERE deleted_at IS NULL;

DROP TRIGGER IF EXISTS loai_van_ban_ba_tang ON loai_van_ban;
CREATE TRIGGER loai_van_ban_ba_tang
    BEFORE UPDATE OR DELETE ON loai_van_ban
