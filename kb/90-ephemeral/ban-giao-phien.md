...
`ok`** (§5, dòng đầu bảng). Xanh ở cổng kiểm **không** có nghĩa là SQL đã chạy.
...
### 2.2 Server gRPC của `identity` — **chưa tồn tại**
...
### 2.3 `platform`: `danh_muc` + `loi_he_thong`

Chưa có. Không bị chặn bởi câu hỏi nào.

### 2.4 Màn hình xác thực lời khai cư trú
...
| **Khách** | #19 #20 | Công dân sửa lời khai đã xác thực · tên khoá quyền cho việc xác thực. Chặn §2.4 |
...
`.claude/commands/` · `kb/90-ephemeral/`.

**Chỗ hai phạm vi sắp chạm nhau:** khi sáu bảng kênh công dân xong, `service-identity/internal/
store/` sẽ cần kho đọc cho chúng — và đó là vùng của phiên này.

> Mục này **phải xoá hẳn** khi không còn phiên nào khác, chứ không để lại. Một mục §4 trỏ vào
> một phiên đã kết thúc còn tệ hơn không có mục §4.
...
| Hook báo nhầm | Đã vá bốn lần. Nếu gặp lần nữa: **sửa hook + thêm ca test, đừng đi vòng** — hook nhiễu là hook bị tắt |
