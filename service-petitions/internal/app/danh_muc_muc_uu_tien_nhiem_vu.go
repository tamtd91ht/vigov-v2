...
// the customer's call, not a use case's — see migrations/0003_danh_muc_muc_uu_tien_nhiem_vu.sql:47-52. An
...
// KhoMucUuTien is the store, declared at the point of use.
//
...
type KhoMucUuTien interface {
	TheoIDDeSua(ctx context.Context, tx *store.ScopedTx, id string) (domain.MucUuTienNhiemVu, error)
	...
	Chen(ctx context.Context, tx *store.ScopedTx, muu domain.MucUuTienNhiemVu) error
	BoMacDinhKhac(ctx context.Context, tx *store.ScopedTx, trongID string) error
	CapNhat(ctx context.Context, tx *store.ScopedTx, muu domain.MucUuTienNhiemVu) error
	XoaMem(ctx context.Context, tx *store.ScopedTx, id, boi, lyDo string) error
}

// DanhMucMucUuTien owns adding, editing and retiring one commune's document types.
type DanhMucMucUuTien struct {
	db  *store.DB
	kho KhoMucUuTien
...
func NewDanhMucMucUuTien(db *store.DB, kho KhoMucUuTien) *DanhMucMucUuTien {
	return &DanhMucMucUuTien{db: db, kho: kho, sinhID: ulid.Moi}
}
...
// been edited again.
const (
	HanhViThemMucUuTien = "them_muc_uu_tien_nhiem_vu"
	HanhViSuaMucUuTien  = "sua_muc_uu_tien_nhiem_vu"
	HanhViXoaMucUuTien  = "xoa_muc_uu_tien_nhiem_vu"
)

// YeuCauThemMucUuTien is one new row, as it arrives from the handler.
//
...
type YeuCauThemMucUuTien struct {
	Ma        string
	Nhan      string
	ThuTu     int
	LaMacDinh bool
...
type YeuCauSuaMucUuTien struct {
	Nhan      *string
	ThuTu     *int
	DangDung  *bool
	LaMacDinh *bool
...
func (uc *DanhMucMucUuTien) Them(ctx context.Context, yc YeuCauThemMucUuTien,
	...
	id, err := uc.sinhID()
	if err != nil {
		return domain.MucUuTienNhiemVu{}, fmt.Errorf("danh_muc_muc_uu_tien_nhiem_vu: sinh mã: %w", err)
	}
	...
	err = uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if n >= docstore.TranDanhMucMucUuTien {
		...
		delta, err := json.Marshal(map[string]any{"sau": tomTatMucUuTien(moi)})
		if err != nil {
			return fmt.Errorf("danh_muc_muc_uu_tien_nhiem_vu: mã hoá delta: %w", err)
		}
	...
	if err != nil {
		...
		return domain.MucUuTienNhiemVu{}, boc(ctx, "thêm", err)
	}
	return moi, nil
}
...
func (uc *DanhMucMucUuTien) Sua(ctx context.Context, id string, yc YeuCauSuaMucUuTien,
	nguoi audit.Actor) (domain.MucUuTienNhiemVu, error) {

	if id == "" {
		return domain.MucUuTienNhiemVu{}, docstore.ErrDanhMucKhongTonTai
	}
	...
	err := uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if khongDoiMucUuTien(truoc, sau) {
			return nil
		}
		...
		delta, err := json.Marshal(map[string]any{
			"truoc": tomTatDoiMucUuTien(truoc, sau, true),
			"sau":   tomTatDoiMucUuTien(truoc, sau, false),
		})
		if err != nil {
			return fmt.Errorf("danh_muc_muc_uu_tien_nhiem_vu: mã hoá delta: %w", err)
		}
		return audit.Write(ctx, tx, audit.Entry{
			Actor:   nguoi,
			Action:  HanhViSuaMucUuTien,
			Subject: sau.Ma,
			Delta:   delta,
		})
	})
	if err != nil {
		return domain.MucUuTienNhiemVu{}, boc(ctx, "sửa", err)
	}
	return sau, nil
}
...
func (uc *DanhMucMucUuTien) Xoa(ctx context.Context, id, lyDoTho string, nguoi audit.Actor) error {
	if id == "" {
	...
	if nguoi.ID == "" {
		...
		return fmt.Errorf("danh_muc_muc_uu_tien_nhiem_vu: thiếu người xoá")
	}
...
func tomTatMucUuTien(l domain.MucUuTienNhiemVu) map[string]any {
...
func tomTatDoiMucUuTien(truoc, sau domain.MucUuTienNhiemVu, ben bool) map[string]any {
	ra := map[string]any{}
