...
// erases exactly that. Running docstore.LoaiNhiemVuStore unchanged over this driver keeps the
...
var (
	xaLoaiNhiemVuA = tenant.ID("01JA" + strings.Repeat("A", 22))
	xaLoaiNhiemVuB = tenant.ID("01JB" + strings.Repeat("B", 22))
)

type lenhLoaiNhiemVuGhi struct {
	sql  string
	args []driver.Value
}

// hangLoaiNhiemVu is the row the FOR UPDATE read hands back. A nil *hangLoaiNhiemVu means "no such live row".
type hangLoaiNhiemVu struct {
	id, ma, nhan string
	dangDung     bool
	macDinh      bool
	thuTu        int
...
func (k *khoLoaiNhiemVuGia) ghi(q string, args []driver.NamedValue) {
	gt := make([]driver.Value, 0, len(args))
	for _, a := range args {
		gt = append(gt, a.Value)
	}
	k.mu.Lock()
	k.lenh = append(k.lenh, lenhLoaiNhiemVuGhi{sql: q, args: gt})
	k.mu.Unlock()
}
...
func (k *khoLoaiNhiemVuGia) cau(tu string) []lenhLoaiNhiemVuGhi {
	k.mu.Lock()
	defer k.mu.Unlock()
	var ra []lenhLoaiNhiemVuGhi
...
func (k *khoLoaiNhiemVuGia) coCau(tu string) bool { return len(k.cau(tu)) > 0 }

// kiemLoi decides whether this statement is the one that fails.
func (k *khoLoaiNhiemVuGia) kiemLoi(q string) error {
	if k.loi != nil {
		return k.loi
	}
	k.mu.Lock()
...
func (k *khoLoaiNhiemVuGia) Connect(context.Context) (driver.Conn, error) {
	return &connLoaiNhiemVuGia{k: k}, nil
}
func (k *khoLoaiNhiemVuGia) Driver() driver.Driver { return trinhLoaiNhiemVuGia{} }

type trinhLoaiNhiemVuGia struct{}

func (trinhLoaiNhiemVuGia) Open(string) (driver.Conn, error) {
	return nil, errors.New("driver giả: chỉ dùng Connector")
}

type connLoaiNhiemVuGia struct{ k *khoLoaiNhiemVuGia }

func (c *connLoaiNhiemVuGia) Prepare(string) (driver.Stmt, error) {
	return nil, errors.New("driver giả: không hỗ trợ Prepare")
}
func (c *connLoaiNhiemVuGia) Close() error { return nil }

func (c *connLoaiNhiemVuGia) Begin() (driver.Tx, error) {
	return c.BeginTx(context.Background(), driver.TxOptions{})
}

func (c *connLoaiNhiemVuGia) BeginTx(context.Context, driver.TxOptions) (driver.Tx, error) {
	c.k.mu.Lock()
	c.k.batDau++
	c.k.mu.Unlock()
	return &txLoaiNhiemVuGia{k: c.k}, nil
}

func (c *connLoaiNhiemVuGia) ExecContext(_ context.Context, q string, args []driver.NamedValue) (driver.Result, error) {
	c.k.ghi(q, args)
...
func (c *connLoaiNhiemVuGia) QueryContext(_ context.Context, q string, args []driver.NamedValue) (driver.Rows, error) {
	c.k.ghi(q, args)
	if err := c.k.kiemLoi(q); err != nil {
		return nil, err
	}
	...
	case strings.Contains(q, "count(*)") && strings.Contains(q, "ma = $2"):
		return &rowsLoaiNhiemVuGia{cot: []string{"count"}, hang: [][]driver.Value{{int64(c.k.maTrung)}}}, nil
	case strings.Contains(q, "count(*)"):
		return &rowsLoaiNhiemVuGia{cot: []string{"count"}, hang: [][]driver.Value{{int64(c.k.dem)}}}, nil
	case strings.Contains(q, "FOR UPDATE"):
		if c.k.hang == nil {
			return &rowsLoaiNhiemVuGia{cot: cotLoaiNhiemVuGia()}, nil
		}
		h := *c.k.hang
...
// cotLoaiNhiemVuGia mirrors docstore's column list ORDER. Written out here rather than imported so that
...
func (t *txLoaiNhiemVuGia) Commit() error {
	t.k.mu.Lock()
	t.k.daCommit++
	t.k.mu.Unlock()
	return nil
...
func (t *txLoaiNhiemVuGia) Rollback() error {
	t.k.mu.Lock()
	t.k.daRollback++
	t.k.mu.Unlock()
	return nil
...
type rowsLoaiNhiemVuGia struct {
	cot  []string
	hang [][]driver.Value
	i    int
}

func (r *rowsLoaiNhiemVuGia) Columns() []string { return r.cot }
func (r *rowsLoaiNhiemVuGia) Close() error      { return nil }
func (r *rowsLoaiNhiemVuGia) Next(dest []driver.Value) error {
	if r.i >= len(r.hang) {
		return io.EOF
	}
...
// dungUseCaseLoaiNhiemVu builds the REAL use case over the REAL store over the fake driver, with the id pinned
// so assertions can name it.
func dungUseCaseLoaiNhiemVu(t *testing.T, k *khoLoaiNhiemVuGia) (*DanhMucLoaiNhiemVu, context.Context) {
	t.Helper()
	db := sql.OpenDB(k)
	...
	kho := store.New(db)
	uc := NewDanhMucLoaiNhiemVu(kho, docstore.NewLoaiNhiemVuStore(kho))
	uc.sinhID = func() (string, error) { return "01JIDMOICUADONGVUATAO0000", nil }
	return uc, tenant.Into(context.Background(), xaLoaiNhiemVuA)
}
