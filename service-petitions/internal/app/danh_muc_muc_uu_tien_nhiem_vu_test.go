...
func themMauMucUuTien() YeuCauThemMucUuTien {
	return YeuCauThemMucUuTien{Ma: "bao-cao", Nhan: "Báo cáo", ThuTu: 5}
}

// dongTang1 is a row the commune added itself — tier 1, the only tier that may be deleted.
func dongTang1MucUuTien() *hangMucUuTien {
	return &hangMucUuTien{id: "muu-001", ma: "bao-cao", nhan: "Báo cáo", dangDung: true,
		thuTu: 5, nguon: domain.NguonDonVi}
}

// dongTang2 ships with the software: it may be disabled and relabelled, never deleted.
func dongTang2MucUuTien() *hangMucUuTien {
	return &hangMucUuTien{id: "muu-002", ma: "cong-van", nhan: "Công văn", dangDung: true,
		thuTu: 1, nguon: domain.NguonHeThong}
}

// dongTang3 is a system row the SOURCE CODE BRANCHES ON. Relabelling is all that is left.
func dongTang3MucUuTien() *hangMucUuTien {
	return &hangMucUuTien{id: "muu-003", ma: "quyet-dinh", nhan: "Quyết định", dangDung: true,
		thuTu: 2, nguon: domain.NguonHeThong, reNhanh: true}
}

// --- rule 6, invariant 3: ONE transaction --------------------------------------------------------

func TestThemGhiDongVaVetTrongCUNGMotGiaoDich_MucUuTien(t *testing.T) {
	...
	if !k.coCau("INSERT INTO muc_uu_tien_nhiem_vu") {
...
func TestThemVetMangDuNguoiViecVaMa_MucUuTien(t *testing.T) {
...
func TestThemKhongGhiGiKhiVetHong_MucUuTien(t *testing.T) {
	...
	k := &khoMucUuTienGia{loiSau: "INSERT INTO audit_log"}
	uc, ctx := dungUseCaseMucUuTien(t, k)
...
func TestXoaChiChoTang1_MucUuTien(t *testing.T) {
	for ten, tc := range map[string]struct {
		hang *hangMucUuTien
		loi  error
	}{
		"tầng 1 xoá được":       {dongTang1MucUuTien(), nil},
		"tầng 2 không xoá được": {dongTang2MucUuTien(), domain.ErrKhongXoaDuocMucHeThong},
		"tầng 3 không xoá được": {dongTang3MucUuTien(), domain.ErrKhongXoaDuocMucHeThong},
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoMucUuTienGia{hang: tc.hang}
			uc, ctx := dungUseCaseMucUuTien(t, k)

			err := uc.Xoa(ctx, tc.hang.id, "gộp vào loại khác", nguoiThuMucUuTien())
			if tc.loi == nil {
				if err != nil {
			...
			if k.coCau("UPDATE muc_uu_tien_nhiem_vu") {
...
func TestTatChiBiTuChoiOTang3_MucUuTien(t *testing.T) {
	...
	for ten, tc := range map[string]struct {
		...
		"tầng 1 tắt được":       {dongTang1MucUuTien(), true},
		"tầng 2 tắt được":       {dongTang2MucUuTien(), true},
		"tầng 3 không tắt được": {dongTang3MucUuTien(), false},
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoMucUuTienGia{hang: tc.hang}
			uc, ctx := dungUseCaseMucUuTien(t, k)

			_, err := uc.Sua(ctx, tc.hang.id, YeuCauSuaMucUuTien{DangDung: &sai}, nguoiThuMucUuTien())
			if tc.duoc {
				if err != nil {
					t.Fatalf("tắt lỗi: %v", err)
				}
				if !k.coCau("UPDATE muc_uu_tien_nhiem_vu") || !k.coCau("INSERT INTO audit_log") {
...
func TestSuaNhanDuocOMoiTang_MucUuTien(t *testing.T) {
	...
	nhan := "Quyết định (sửa)"
	for ten, hang := range map[string]*hangMucUuTien{
		"tầng 1": dongTang1MucUuTien(), "tầng 2": dongTang2MucUuTien(), "tầng 3": dongTang3MucUuTien(),
	} {
		t.Run(ten, func(t *testing.T) {
			k := &khoMucUuTienGia{hang: hang}
			uc, ctx := dungUseCaseMucUuTien(t, k)

			sau, err := uc.Sua(ctx, hang.id, YeuCauSuaMucUuTien{Nhan: &nhan}, nguoiThuMucUuTien())
			if err != nil {
...
func TestChenKhongCoThamSoNaoChoNguon_MucUuTien(t *testing.T) {
...
func TestCapNhatKhongDungToiMaVaNguon_MucUuTien(t *testing.T) {
	...
	nhan := "Công văn mới"
	k := &khoMucUuTienGia{hang: dongTang2MucUuTien()}
	uc, ctx := dungUseCaseMucUuTien(t, k)

	if _, err := uc.Sua(ctx, "muu-002", YeuCauSuaMucUuTien{Nhan: &nhan}, nguoiThuMucUuTien()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	capNhat := k.cau("UPDATE muc_uu_tien_nhiem_vu")
	if len(capNhat) != 1 {
...
func TestXoaLaXoaMemVaGhiDuBaCot_MucUuTien(t *testing.T) {
	...
	if err := uc.Xoa(ctx, "muu-001", "gộp vào loại khác", nguoiThuMucUuTien()); err != nil {
		t.Fatalf("Xoá lỗi: %v", err)
	}
...
func TestMoiDuongDocGhiDeuLoaiDongDaXoaMem_MucUuTien(t *testing.T) {
	...
	nhan := "Báo cáo mới"
	k2 := &khoMucUuTienGia{hang: dongTang1MucUuTien()}
	uc2, ctx2 := dungUseCaseMucUuTien(t, k2)
	if _, err := uc2.Sua(ctx2, "muu-001", YeuCauSuaMucUuTien{Nhan: &nhan}, nguoiThuMucUuTien()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	for _, l := range k2.cau("UPDATE muc_uu_tien_nhiem_vu") {
...
func TestSuaDongDaXoaLa404_MucUuTien(t *testing.T) {
	...
	nhan := "Báo cáo mới"
	k := &khoMucUuTienGia{hang: nil}
	uc, ctx := dungUseCaseMucUuTien(t, k)

	if _, err := uc.Sua(ctx, "muu-001", YeuCauSuaMucUuTien{Nhan: &nhan}, nguoiThuMucUuTien()); !errors.Is(err, docstore.ErrDanhMucKhongTonTai) {
...
func TestXoaDoiLyDo_MucUuTien(t *testing.T) {
	...
	if err := uc.Xoa(ctx, "muu-001", "   ", nguoiThuMucUuTien()); !errors.Is(err, domain.ErrThieuLyDoXoa) {
...
func TestThemTuChoiMaDaCoKeCaDongDaXoa_MucUuTien(t *testing.T) {
	...
	k := &khoMucUuTienGia{maTrung: 1}
	uc, ctx := dungUseCaseMucUuTien(t, k)

	_, err := uc.Them(ctx, themMauMucUuTien(), nguoiThuMucUuTien())
...
func TestThemTuChoiKhiDayTran_MucUuTien(t *testing.T) {
	// A CREATE HAS TO CARE ABOUT THE READ ROUTE'S CEILING. DanhSach REFUSES rather than truncates
	// past TranDanhMucMucUuTien, so the row that crosses the line does not merely add itself — it
	// turns the whole catalogue into a 500 for every screen in the commune.
	k := &khoMucUuTienGia{dem: docstore.TranDanhMucMucUuTien}
	uc, ctx := dungUseCaseMucUuTien(t, k)
	...
	// perfectly valid.
	k2 := &khoMucUuTienGia{dem: docstore.TranDanhMucMucUuTien - 1}
	uc2, ctx2 := dungUseCaseMucUuTien(t, k2)
	if _, err := uc2.Them(ctx2, themMauMucUuTien(), nguoiThuMucUuTien()); err != nil {
...
func TestSuaKhongDoiGiThiKhongGhiVaKhongVet_MucUuTien(t *testing.T) {
	...
	nhan := "Báo cáo" // exactly what dongTang1MucUuTien() already holds
	k := &khoMucUuTienGia{hang: dongTang1MucUuTien()}
	uc, ctx := dungUseCaseMucUuTien(t, k)

	sau, err := uc.Sua(ctx, "muu-001", YeuCauSuaMucUuTien{Nhan: &nhan}, nguoiThuMucUuTien())
	if err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	if sau.Nhan != "Báo cáo" {
...
func TestSuaDatMacDinhBoMacDinhCuTruocTrongCungGiaoDich_MucUuTien(t *testing.T) {
	...
	if _, err := uc.Sua(ctx, "muu-001", YeuCauSuaMucUuTien{LaMacDinh: &dung}, nguoiThuMucUuTien()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
...
func TestMoiCauLenhMangXaTuContext_MucUuTien(t *testing.T) {
	...
	// statement forgot.
	for _, xa := range []string{string(xaMucUuTienA), string(xaMucUuTienB)} {
		k := &khoMucUuTienGia{}
		uc, _ := dungUseCaseMucUuTien(t, k)
...
func TestKhongCoXaTrongContextThiPanic_MucUuTien(t *testing.T) {
	...
	k := &khoMucUuTienGia{}
	uc, _ := dungUseCaseMucUuTien(t, k)
	_, _ = uc.Them(context.Background(), themMauMucUuTien(), nguoiThuMucUuTien())
}
...
func chuoiTrongDoiMucUuTien(args []driver.Value) map[string]bool {
	ra := map[string]bool{}
	for _, a := range args {
		switch v := a.(type) {
		case string:
