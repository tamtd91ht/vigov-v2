...
// KhoLoaiTaiNguyen is the store, declared at the point of use.
//
...
type KhoLoaiTaiNguyen interface {
	TheoIDDeSua(ctx context.Context, tx *store.ScopedTx, id string) (domain.LoaiTaiNguyenBanDo, error)
	...
	Chen(ctx context.Context, tx *store.ScopedTx, ltn domain.LoaiTaiNguyenBanDo) error
...
type DanhMucLoaiTaiNguyen struct {
	db  *store.DB
	kho KhoLoaiTaiNguyen

	// sinhID is injected so a test can pin the id. In production it is ulid.Moi.
	sinhID func() (string, error)
}

func NewDanhMucLoaiTaiNguyen(db *store.DB, kho KhoLoaiTaiNguyen) *DanhMucLoaiTaiNguyen {
	return &DanhMucLoaiTaiNguyen{db: db, kho: kho, sinhID: ulid.Moi}
}
...
// been edited again.
const (
	HanhViThemLoaiTaiNguyen = "them_loai_tai_nguyen_ban_do"
	HanhViSuaLoaiTaiNguyen  = "sua_loai_tai_nguyen_ban_do"
	HanhViXoaLoaiTaiNguyen  = "xoa_loai_tai_nguyen_ban_do"
)

// YeuCauThemLoaiTaiNguyen is one new row, as it arrives from the handler.
//
...
type YeuCauThemLoaiTaiNguyen struct {
	Ma        string
	Nhan      string
	ThuTu     int
	LaMacDinh bool
...
type YeuCauSuaLoaiTaiNguyen struct {
	Nhan      *string
	ThuTu     *int
	DangDung  *bool
	LaMacDinh *bool
...
func (uc *DanhMucLoaiTaiNguyen) Them(ctx context.Context, yc YeuCauThemLoaiTaiNguyen,
	...
	id, err := uc.sinhID()
	if err != nil {
		return domain.LoaiTaiNguyenBanDo{}, fmt.Errorf("danh_muc_loai_tai_nguyen_ban_do: sinh mã: %w", err)
	}

	moi := domain.LoaiTaiNguyenBanDo{
	...
	err = uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if n >= docstore.TranDanhMucLoaiTaiNguyen {
		...
		delta, err := json.Marshal(map[string]any{"sau": tomTatLoaiTaiNguyen(moi)})
		if err != nil {
		...
		return audit.Write(ctx, tx, audit.Entry{
			Actor:   nguoi,
			Action:  HanhViThemLoaiTaiNguyen,
	...
	if err != nil {
		...
		return domain.LoaiTaiNguyenBanDo{}, boc(ctx, "thêm", err)
	}
	return moi, nil
}
...
func (uc *DanhMucLoaiTaiNguyen) Sua(ctx context.Context, id string, yc YeuCauSuaLoaiTaiNguyen,
	nguoi audit.Actor) (domain.LoaiTaiNguyenBanDo, error) {

	if id == "" {
		return domain.LoaiTaiNguyenBanDo{}, docstore.ErrDanhMucKhongTonTai
	}
	...
	err := uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		if khongDoiLoaiTaiNguyen(truoc, sau) {
			return nil
		}
		...
		delta, err := json.Marshal(map[string]any{
			"truoc": tomTatDoiLoaiTaiNguyen(truoc, sau, true),
			"sau":   tomTatDoiLoaiTaiNguyen(truoc, sau, false),
		})
		if err != nil {
			return fmt.Errorf("danh_muc_loai_tai_nguyen_ban_do: mã hoá delta: %w", err)
		}
		return audit.Write(ctx, tx, audit.Entry{
			Actor:   nguoi,
			Action:  HanhViSuaLoaiTaiNguyen,
			Subject: sau.Ma,
			Delta:   delta,
		})
	})
	if err != nil {
		return domain.LoaiTaiNguyenBanDo{}, boc(ctx, "sửa", err)
	}
	return sau, nil
}
...
func (uc *DanhMucLoaiTaiNguyen) Xoa(ctx context.Context, id, lyDoTho string, nguoi audit.Actor) error {
	if id == "" {
	...
	err = uc.db.For(ctx).Tx(ctx, func(tx *store.ScopedTx) error {
		...
		delta, err := json.Marshal(map[string]any{
			"truoc":   tomTatLoaiTaiNguyen(truoc),
			"ly_do":   lyDo,
			"xoa_mem": true,
		})
		if err != nil {
...
func tomTatLoaiTaiNguyen(l domain.LoaiTaiNguyenBanDo) map[string]any {
...
func tomTatDoiLoaiTaiNguyen(truoc, sau domain.LoaiTaiNguyenBanDo, ben bool) map[string]any {
	ra := map[string]any{}
