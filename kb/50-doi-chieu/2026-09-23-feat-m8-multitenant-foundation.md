...
| `0142452` · migration `0035` | `20260906_0035-0035_fiscal_headline.py` | Cột `fiscal_lines.is_headline` — **người chọn** dòng nào là con số tổng, máy không đoán bừa. Sheet thu có hai dòng cấp cao nhất lồng nhau; sheet chi có "Tổng số" đứng ngang hàng A…E — cộng hết là đếm đôi | **Đã khớp** (sửa 23/09): `service-finance/migrations/0006_thu_chi_ngan_sach.sql:334` `is_headline BOOLEAN NOT NULL DEFAULT false`, và tệp ghi thẳng *"THE HEADLINE ROW IS CHOSEN BY A PERSON"* (`:38`) — cùng một lập luận, không nạp sẵn giá trị. Kèm theo, luật đơn vị tính đã ở `bang_ngan_sach.don_vi_tinh` (`:167`) và nhiều kỳ trong năm ở `bang_ngan_sach.lan` (`:160`). Không có việc đổi lược đồ nào phát sinh từ hai commit này. **⚠ SỬA 25/09 — "đã khớp" chỉ đúng ở LƯỢC ĐỒ.** Cùng commit này còn mang hai luật ĐỌC mà kho này **không** theo: `pick_headline` **đoán** dòng tổng khi chưa ai đánh sao, và `_fiscal_block` lấy cột `actual` **đầu tiên** (= `Thu ngân sách NSNN`) cho cân đối, trái #32. Xem §Đã sửa hàng 7 |
...
### Lượt sửa thứ ba, 25/09/2026 — dòng `0142452` chỉ so lược đồ

Sai **ngược chiều** hai lượt trên: báo *khớp* ở chỗ còn lệch. Chữ cũ ở §M3 giữ nguyên, chỉ thêm dấu SỬA.
Đo trong đợt đối chiếu trước neo ở `kb/50-doi-chieu/2026-09-25-feat-m8-multitenant-foundation.md` §B.

| # | Chỗ | Bản 23/09 viết | Sự thật, có `file:line` |
|---|---|---|---|
| 7 | §M3, dòng `0142452` | *"**Đã khớp** … Không có việc đổi lược đồ nào phát sinh từ hai commit này"* | Đúng ở **lược đồ** (`0006_thu_chi_ngan_sach.sql:334`), nhưng `0142452` còn thêm ba hàm đọc mà dòng ấy không nhắc. (a) `pick_headline` (`apps/api/app/modules/budget/fiscal_service.py:497-531` tại `0053854`): chưa ai đánh sao thì **đoán** dòng cấp cao nhất có số thực hiện lớn nhất. Kho này **không đoán**: không có dòng đánh dấu thì không có tổng (`0006:38-66`), nhiều dòng đánh dấu thì từ chối (`service-finance/internal/domain/thu_chi_ngan_sach.go:696-725`). Kho này đúng hướng ADR 0035 §A, không có việc. (b) `headline_figures` + `_fiscal_block` (`fiscal_service.py:952-980`, `apps/api/app/modules/reports/service.py:361-406`): tử số cân đối là cột vai `actual` **đầu tiên**, trên mẫu Thăng Bình là `Thu ngân sách NSNN`. **Trái #32 `DECIDED`** (`thu_chi_ngan_sach.go:90-96,881-888` lấy `thu-xa-huong`) → mâu thuẫn, ghi ở §Mâu thuẫn của ghi chú 25/09. (c) `_to_dong` (`reports/service.py:337-358`): chỉ quy về đồng với 4 chữ đơn vị đã biết, chữ lạ thì bỏ số tuyệt đối. Kho này lưu đồng ngay từ đầu (`0006:13-18`), nên không mang luật ấy sang |

Nhân tiện, câu còn mở ở Tóm tắt mục 7 (*"tuyến GHI có cưỡng chế `cach_tinh='children'` chưa"*) đã có
lời đáp trong mã: sửa số một dòng có con trả 409 (`service-finance/internal/http/routes.go:1035-1040`,
`internal/app/thu_chi_ngan_sach.go:584-590` gọi `domain.ChoGhiGiaTri`).
