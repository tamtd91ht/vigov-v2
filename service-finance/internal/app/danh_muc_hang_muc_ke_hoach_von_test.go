...
func themMau() YeuCauThemHangMuc {
	return YeuCauThemHangMuc{Ma: "bao-cao", Nhan: "Báo cáo", ThuTu: 5}
}

// dongTang1 is a row the commune added itself — tier 1, the only tier that may be deleted.
func dongTang1() *hangLVB {
	return &hangLVB{id: "hm-001", ma: "bao-cao", nhan: "Báo cáo", dangDung: true,
...
func dongTang2() *hangLVB {
	return &hangLVB{id: "hm-002", ma: "cong-van", nhan: "Công văn", dangDung: true,
...
func dongTang3() *hangLVB {
	return &hangLVB{id: "hm-003", ma: "quyet-dinh", nhan: "Quyết định", dangDung: true,
...
func TestThemGhiDongVaVetTrongCUNGMotGiaoDich(t *testing.T) {
	...
	if !k.coCau("INSERT INTO hang_muc_ke_hoach_von") {
...
func TestXoaChiChoTang1(t *testing.T) {
	for ten, tc := range map[string]struct {
		...
		t.Run(ten, func(t *testing.T) {
			...
			if k.coCau("UPDATE hang_muc_ke_hoach_von") {
...
func TestTatChiBiTuChoiOTang3(t *testing.T) {
	...
	for ten, tc := range map[string]struct {
		...
		t.Run(ten, func(t *testing.T) {
			...
			_, err := uc.Sua(ctx, tc.hang.id, YeuCauSuaHangMuc{DangDung: &sai}, nguoiThu())
			if tc.duoc {
				if err != nil {
					t.Fatalf("tắt lỗi: %v", err)
				}
				if !k.coCau("UPDATE hang_muc_ke_hoach_von") || !k.coCau("INSERT INTO audit_log") {
...
func TestSuaNhanDuocOMoiTang(t *testing.T) {
	...
	for ten, hang := range map[string]*hangLVB{
		...
		t.Run(ten, func(t *testing.T) {
			...
			sau, err := uc.Sua(ctx, hang.id, YeuCauSuaHangMuc{Nhan: &nhan}, nguoiThu())
			if err != nil {
...
func TestCapNhatKhongDungToiMaVaNguon(t *testing.T) {
	...
	if _, err := uc.Sua(ctx, "hm-002", YeuCauSuaHangMuc{Nhan: &nhan}, nguoiThu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	capNhat := k.cau("UPDATE hang_muc_ke_hoach_von")
	if len(capNhat) != 1 {
...
func TestXoaLaXoaMemVaGhiDuBaCot(t *testing.T) {
	...
	if err := uc.Xoa(ctx, "hm-001", "gộp vào loại khác", nguoiThu()); err != nil {
		t.Fatalf("Xoá lỗi: %v", err)
	}
...
func TestMoiDuongDocGhiDeuLoaiDongDaXoaMem(t *testing.T) {
	...
	if _, err := uc2.Sua(ctx2, "hm-001", YeuCauSuaHangMuc{Nhan: &nhan}, nguoiThu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	for _, l := range k2.cau("UPDATE hang_muc_ke_hoach_von") {
...
func TestSuaDongDaXoaLa404(t *testing.T) {
	...
	if _, err := uc.Sua(ctx, "hm-001", YeuCauSuaHangMuc{Nhan: &nhan}, nguoiThu()); !errors.Is(err, docstore.ErrDanhMucKhongTonTai) {
...
func TestThemTuChoiKhiDayTran(t *testing.T) {
	...
	// past TranDanhMucHangMuc, so the row that crosses the line does not merely add itself — it
	// turns the whole catalogue into a 500 for every screen in the commune.
	k := &khoGia{dem: docstore.TranDanhMucHangMuc}
	uc, ctx := dungUseCase(t, k)
	...
	// perfectly valid.
	k2 := &khoGia{dem: docstore.TranDanhMucHangMuc - 1}
	uc2, ctx2 := dungUseCase(t, k2)
...
func TestSuaKhongDoiGiThiKhongGhiVaKhongVet(t *testing.T) {
	...
	sau, err := uc.Sua(ctx, "hm-001", YeuCauSuaHangMuc{Nhan: &nhan}, nguoiThu())
	if err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
	if sau.Nhan != "Báo cáo" {
...
func TestSuaDatMacDinhBoMacDinhCuTruocTrongCungGiaoDich(t *testing.T) {
	...
	if _, err := uc.Sua(ctx, "hm-001", YeuCauSuaHangMuc{LaMacDinh: &dung}, nguoiThu()); err != nil {
		t.Fatalf("Sua lỗi: %v", err)
	}
