...
import (
	...
	svchttp "github.com/vihat/vigov/service-dossiers/internal/http"
	"github.com/vihat/vigov/service-dossiers/migrations"
)
...
func main() {
	...
	if err != nil {
		log.Error("migration không chạy được", "service", "dossiers", "err", err)
		os.Exit(1)
	}
	...
	addr := cfg.ListenAddrHoac(":8085")
	log.Info("starting", "service", "dossiers", "addr", addr)
...
func chayMigration(log *slog.Logger) (config.Config, error) {
	...
	if err := db.PingContext(ctx); err != nil {
		return config.Config{}, fmt.Errorf("dossiers: không nối được cơ sở dữ liệu: %w", err)
	}

	kq, err := migrate.Chay(ctx, db, migrations.FS, "dossiers")
	if err != nil {
		return config.Config{}, err
	}
	...
	// prompt.
	log.Info("migration xong", "service", "dossiers", "da_ap", kq.DaAp, "bo_qua", len(kq.BoQua))
	return cfg, nil
}
