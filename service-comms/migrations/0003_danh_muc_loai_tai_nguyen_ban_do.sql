-- comms — the reference catalogue comms owns: map asset type (ADR 0024).
--
...
--      see "WHERE THE he-thong ROWS COME FROM" below, which in this service has a SECOND reason
--      on top of the general one. The ceiling in sight is around a dozen rows per commune.
...
--      and then the first map asset that stores one of these codes as a value.
--   5. RETENTION: nothing is removed. An asset record keeps the type it was filed under, and
--      the custom-field set of the map hangs off that type, so rows are soft deleted and hard
...
--
-- THIS FILE CREATES THE TABLE AND SEEDS NOTHING, for two separate reasons. The first applies to
-- every catalogue in the system; the second applies only here and would hold even if the first
-- were solved tomorrow.
--
-- REASON ONE — a seed row belongs to a commune, and this code path has no commune in it.
--
...
--
--   So system rows are seed sown PER COMMUNE, and the step that sows them is COMMUNE
--   ONBOARDING. THAT STEP DOES NOT EXIST IN THIS REPOSITORY YET, and designing it is not a
--   decision a migration may take on its own: it fixes who writes a commune's first rows, under
--   which principal in the audit trail (rule 6, invariant 6), and what happens to the communes
--   already onboarded when the shipped list later changes. That question is raised with the
--   user; it is NOT settled here.
--
-- REASON TWO — THE SPECIFICATION CONTRADICTS ITSELF ABOUT THIS LIST, so there is no list to sow.
--
--   * docs/ui-ux/10-ban-do-kinh-te-so.md:37 heads a table of ELEVEN asset groups.
--   * docs/ui-ux/10-ban-do-kinh-te-so.md:53 says the `Loại tài nguyên bản đồ` catalogue holds
--     EIGHT, that the other three (`coop`, `tourism`, `public_project`) are hard-coded in the
--     prototype, and instructs: "khi dựng lại: đưa cả 11 nhóm vào danh mục".
--   * The two lists do not even use the same codes: :41–:51 spells them in English
--     (`enterprise`, `household_business`), :53 in Vietnamese slugs (`doanh-nghiep`,
--     `ho-kinh-doanh`).
--
--   Sowing the eight and letting somebody add the missing three by hand later is the reliable
--   way to end up with two sources for one list, which is the failure rule 9 describes and the
--   one that cannot be undone quietly once communes have assets filed under either spelling.
--   Both the COUNT and the SPELLING have to be decided by the user before a single row is sown.
--
-- WHAT HOLDS UNTIL THEN: this table is empty for every commune. A catalogue screen reading it
-- shows an empty list — visibly empty, which is the failure people report, unlike a half-seeded
...
--     and a commune can soft-delete `cho` and create a new, unrelated `cho` — every asset filed
--     under the old value then silently reads as the new one, including the custom fields that
--     hang off the type.
--
...
--   `ma`              — an issued code is never renumbered (rule 7, invariant 3). Asset records
--                       and the map's custom-field definitions hold this code as a VALUE and
...
BEGIN
    ...
    IF NEW.ma IS DISTINCT FROM OLD.ma THEN
        RAISE EXCEPTION 'catalogue %: `ma` is immutable', TG_TABLE_NAME
            USING HINT = 'An issued code is never renumbered (rule 7, invariant 3). Asset '
...
-- @scope:  tenant
--
-- loai_tai_nguyen_ban_do — the groups the economic map is organised by.
--
-- Owned by comms because the map layers and the asset records live here; the catalogue only
-- means anything standing next to them (ADR 0024).
--
-- THE ENTITY IS NAMED "MapAsset…" BECAUSE `asset` IS ALREADY THE WORD THIS SYSTEM USES for
-- these records: the permissions `asset.read` / `asset.update` are settled
-- (docs/ui-ux/10-ban-do-kinh-te-so.md:255). Calling the table's entity `MapResourceType` or
-- `PoiType` would give one concept two English words on two surfaces, which is the drift
-- ADR 0017 is about.
--
-- TIER 2 FOR THE SHIPPED CODES, on the evidence available: the field set of a group is data
-- (`truong_tuy_bien_ban_do`, docs/ui-ux/14-cau-hinh.md §6), not a branch in the code, so adding
-- or removing a group adds or removes a layer and nothing else. The three groups the prototype
-- hard-codes are the ones to watch: if the rebuild keeps any of them special-cased in code
-- rather than driven by data, that code is a branch and its row belongs in tier 3.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS loai_tai_nguyen_ban_do (
    tenant_id         TEXT        NOT NULL,
    id                TEXT        NOT NULL,
    ma                TEXT        NOT NULL,          -- "doanh-nghiep"
    nhan              TEXT        NOT NULL,          -- "Doanh nghiệp"
    ...
    deleted_by        TEXT,
    delete_reason     TEXT,
    -- EXACTLY ONE DEFAULT PER COMMUNE — the map's group selector opens on it. Two defaults make
    -- that choice depend on read order, which is to say random, and nothing on the screen shows
    -- that it is.
    --
...
DO $$ BEGIN
    FOR i IN 0..31 LOOP
        EXECUTE format(
            'CREATE TABLE IF NOT EXISTS loai_tai_nguyen_ban_do_p%s PARTITION OF loai_tai_nguyen_ban_do '
...
-- (rule 7, invariant 2).
CREATE INDEX IF NOT EXISTS loai_tai_nguyen_ban_do_danh_sach
    ON loai_tai_nguyen_ban_do (tenant_id, thu_tu) WHERE deleted_at IS NULL;

DROP TRIGGER IF EXISTS loai_tai_nguyen_ban_do_ba_tang ON loai_tai_nguyen_ban_do;
CREATE TRIGGER loai_tai_nguyen_ban_do_ba_tang
    BEFORE UPDATE OR DELETE ON loai_tai_nguyen_ban_do
